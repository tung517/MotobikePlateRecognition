...
X_train, X_validate, X_test = np.split(X, [int(0.8 * len(X)), int(0.9 * len(X))])
y_train, y_validate, y_test = np.split(y, [int(0.8 * len(y)), int(0.9 * len(y))])
...
model.add(Conv2D(32, (3, 3), input_shape=[64, 32, 1], activation='relu', padding='same'))
